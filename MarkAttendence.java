...
//                    }
                    ...
                    if (nEigens > 0) {
                        ...
                        System.out.println("fromcam heheheheh 1");
                    }
                }

                // Show the data on the screen.
                shownImg = cvCloneImage(camImg);
                System.out.println("fromcam heheheheh 2");
                ...
                if (faceRect.width() > 0)// Check if a face was detected.
                {
                    ...
                    if (nEigens > 0 && confidence >= MaxConfidence) // Check if the face recognition database is loaded and a person was recognized.
                    {
                        ...
                        System.out.println("fromcam heheheheh 6.");

                        String pname = getOrgnlName(name);
                        String std = arr[1];
                        attendence.MarkAttendence(attendence.getSid(pname, std));
                        System.out.println("fromcam heheheheh 7." + pname + std);
                        TextFileWriter textFileWriter = new TextFileWriter();
                        textFileWriter.createtxt(pname);
                ...
                System.out.println("fromcam heheheheh 8.");
                try {
                    // Existing code to mark attendance and write to text file...
                    System.out.println("fromcam heheheheh 8.");
                    // Display a message indicating that attendance is marked
                    Thread.sleep(5000);
...
//
//        public void insertdata(String pname) {
//
...
//
//            // Prepare SQL query
//            String query = "INSERT INTO your_table_name (pname, date, time) VALUES (?, ?, ?)";
//
//            // Execute the query
...
//    ---------------------------------------------file write---------------------------------------------------------------
    public class TextFileWriter {
        ...
        public void createtxt(String sid) {
            String currentDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()); // Current date and time

            // Specify the file path
            String filePath = "result/attendance.txt";
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(filePath, true))) {
                // Append data to the file on a new line
                writer.println("SID: " + sid + ", Date and Time: " + currentDate);
...
//            cvReleaseMemStorage(storage);
    ...
    private int findNearestNeighbor(float projectedTestFace[], FloatPointer pConfidencePointer) {
        ...
        for (iTrain = 0; iTrain < nTrainFaces; iTrain++) {
            LOGGER.info("iTrain: " + iTrain + ", nTrainFaces: " + nTrainFaces);
